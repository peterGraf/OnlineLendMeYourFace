...
static void pblCgiSetContentType(char* type)
{
	if (!contentType)
	{
		...
		if (cookie && cookiePath && cookieDomain)
		{
			...
			{
				format = "Set-Cookie: %s%s; Path=%s; Max-Age=31536000; HttpOnly; Secure; SameSite=Strict\n\n";
				printf(format, pblCgiCookieTag, cookie, cookiePath);
				PBL_CGI_TRACE(format, pblCgiCookieTag, cookie, cookiePath);
			}
		}
		else
		{
